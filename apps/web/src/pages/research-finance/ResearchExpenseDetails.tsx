...
const statusStyles: Record<string, string> = {
  ...
  REJECTED: 'bg-red-100 text-red-700', RECORDED: 'bg-emerald-100 text-emerald-700',
  ...
  return (
    <div className="max-w-5xl space-y-6">
      <div className="flex items-center gap-4">
        <Link to="/research-expenses" className="p-2 hover:bg-slate-200 rounded-full transition-colors"><ArrowLeft size={20} className="text-slate-600" /></Link>
        <div><h1 className="text-2xl font-bold text-slate-900">Research Expense</h1></div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
        <div className="p-6 border-b border-slate-200 bg-slate-50 flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${statusStyles[expense.status]}`}>{EXPENSE_STATUS_LABELS[expense.status]}</span>
              <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-700">{BUDGET_CATEGORY_LABELS[expense.category]}</span>
            </div>
            <h2 className="text-xl font-bold text-slate-900">{fmt(Number(expense.amount))}</h2>
          ...
          <div className="flex gap-2">
            ...
            {expense.status === 'DRAFT' && isOwner && (
              <button onClick={() => submitExpense.mutate(id)} disabled={submitExpense.isPending} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700 disabled:opacity-50">
                <Send size={16} /> Submit
              </button>
            )}
        ...
        {canManage && (expense.status === 'SUBMITTED' || expense.status === 'UNDER_REVIEW') && (
          <div className="mx-6 mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
            ...
            <input value={rejectReason} onChange={e => setRejectReason(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500" placeholder="Enter reason for rejection..." />
          </div>
        )}

        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider">Expense Information</h3>
            <div className="space-y-3">
              <div><div className="text-xs font-medium text-slate-500 uppercase">Category</div><div className="text-sm font-medium text-slate-900">{BUDGET_CATEGORY_LABELS[expense.category]}</div></div>
              <div><div className="text-xs font-medium text-slate-500 uppercase">Expense Date</div><div className="text-sm font-medium text-slate-900">{new Date(expense.expenseDate).toLocaleDateString()}</div></div>
              <div><div className="text-xs font-medium text-slate-500 uppercase">Vendor</div><div className="text-sm font-medium text-slate-900">{expense.vendor || 'N/A'}</div></div>
              <div><div className="text-xs font-medium text-slate-500 uppercase">Reference #</div><div className="text-sm font-medium text-slate-900">{expense.referenceNumber || 'N/A'}</div></div>
            </div>
          </div>
          <div className="space-y-4">
            <h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider">Submission Details</h3>
            <div className="space-y-3">
              <div><div className="text-xs font-medium text-slate-500 uppercase">Submitted By</div><div className="text-sm font-medium text-slate-900">{expense.submittedBy?.user?.firstName} {expense.submittedBy?.user?.lastName}</div></div>
              <div><div className="text-xs font-medium text-slate-500 uppercase">Created</div><div className="text-sm font-medium text-slate-900">{new Date(expense.createdAt).toLocaleDateString()}</div></div>
              {expense.approvedBy && <div><div className="text-xs font-medium text-slate-500 uppercase">Approved By</div><div className="text-sm font-medium text-slate-900">{expense.approvedBy.firstName} {expense.approvedBy.lastName}</div>{expense.approvedAt && <div className="text-xs text-slate-500 mt-1">{new Date(expense.approvedAt).toLocaleString()}</div>}</div>}
            </div>
          </div>
        </div>
        ...
        {expense.notes && (
          <div className="mx-6 mb-6"><h3 className="text-sm font-semibold text-slate-900 uppercase tracking-wider mb-2">Notes</h3><p className="text-sm text-slate-600">{expense.notes}</p></div>
        )}
      </div>
