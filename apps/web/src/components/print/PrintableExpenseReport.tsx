...
import { BUDGET_CATEGORY_LABELS, EXPENSE_STATUS_LABELS } from '../../hooks/useResearchFinance';
...
const IconDoc = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={NAVY} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ verticalAlign: 'middle', marginRight: '6px' }}>
...
export const PrintableExpenseReport = React.forwardRef(
  ({ expense }: PrintableExpenseReportProps, ref: React.Ref<HTMLDivElement>) => {
    if (!expense) return null;

    const submitterName = expense.submittedBy?.user
      ? `${expense.submittedBy.user.firstName} ${expense.submittedBy.user.lastName}`
      : '';
    ...
    return (
      ...
      >
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px', fontSize: '12pt' }}>
          <tbody>
            <tr>
              <td style={{ padding: '7px 10px', border: `1px solid ${BORDER}`, fontWeight: 'bold', width: '18%', background: '#f7f7f7' }}>Report Period:</td>
              <td style={{ padding: '7px 10px', border: `1px solid ${BORDER}`, width: '32%' }}>
                {new Date(expense.expenseDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
              </td>
              <td style={{ padding: '7px 10px', border: `1px solid ${BORDER}`, fontWeight: 'bold', width: '18%', background: '#f7f7f7' }}>Category:</td>
              <td style={{ padding: '7px 10px', border: `1px solid ${BORDER}`, width: '32%' }}>
            ...
            <tr>
              ...
              <td style={{ padding: '7px 10px', border: `1px solid ${BORDER}` }}>{submitterName}</td>
            </tr>
            <tr>
              <td style={{ padding: '7px 10px', border: `1px solid ${BORDER}`, fontWeight: 'bold', background: '#f7f7f7' }}>Status:</td>
              <td style={{ padding: '7px 10px', border: `1px solid ${BORDER}` }}>{EXPENSE_STATUS_LABELS[expense.status]}</td>
        ...
        <div style={{ display: 'flex', gap: '16px', marginBottom: '16px' }}>
          ...
          <div style={{ flex: 1, border: `1px solid ${BORDER}`, borderRadius: '5px', overflow: 'hidden' }}>
            <div style={{ background: NAVY, color: '#fff', padding: '8px 12px', fontSize: '12pt', fontWeight: 'bold', display: 'flex', alignItems: 'center' }}>
              <IconChart /> SUMMARY METRICS
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12pt' }}>
              <tbody>
                <tr style={{ background: LIGHT_GREEN }}>
                  <td style={{ padding: '6px 12px', borderBottom: `1px solid ${BORDER}` }}>Expense Code</td>
                  <td style={{ padding: '6px 12px', borderBottom: `1px solid ${BORDER}`, textAlign: 'right', fontWeight: '600' }}>{expense.expenseCode}</td>
                </tr>
                <tr>
                  <td style={{ padding: '6px 12px', borderBottom: `1px solid ${BORDER}` }}>Expense Date</td>
                  <td style={{ padding: '6px 12px', borderBottom: `1px solid ${BORDER}`, textAlign: 'right', fontWeight: '600' }}>
                    {new Date(expense.expenseDate).toLocaleDateString()}
                  </td>
                </tr>
                <tr style={{ background: LIGHT_GREEN }}>
                  <td style={{ padding: '6px 12px', borderBottom: `1px solid ${BORDER}` }}>Vendor / Supplier</td>
                  <td style={{ padding: '6px 12px', borderBottom: `1px solid ${BORDER}`, textAlign: 'right', fontWeight: '600' }}>{expense.vendor || 'N/A'}</td>
                </tr>
                <tr>
                  <td style={{ padding: '6px 12px', borderBottom: `1px solid ${BORDER}` }}>Reference / Invoice #</td>
                  <td style={{ padding: '6px 12px', borderBottom: `1px solid ${BORDER}`, textAlign: 'right', fontWeight: '600' }}>{expense.referenceNumber || 'N/A'}</td>
                </tr>
                <tr style={{ background: '#e4efe4' }}>
                  <td style={{ padding: '8px 12px', fontWeight: 'bold', fontSize: '12pt' }}>TOTAL AMOUNT</td>
                  <td style={{ padding: '8px 12px', textAlign: 'right', fontWeight: 'bold', fontSize: '14pt' }}>{fmt(Number(expense.amount))}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div style={{ flex: 1, border: `1px solid ${BORDER}`, borderRadius: '5px', overflow: 'hidden' }}>
            <div style={{ background: NAVY, color: '#fff', padding: '8px 12px', fontSize: '12pt', fontWeight: 'bold', display: 'flex', alignItems: 'center' }}>
              <IconLink /> LINKED GRANTS & PROJECTS
            </div>
            <div style={{ padding: '10px 12px', fontSize: '12pt' }}>
              <div style={{ marginBottom: '10px' }}>
                <span style={{ fontWeight: 'bold' }}>Research Grant:</span>
                <span style={{ marginLeft: '12px' }}>{expense.researchGrant ? `Grant #${expense.researchGrant.grantNumber}` : 'Not linked'}</span>
                {expense.researchGrant && (
                  <div style={{ fontStyle: 'italic', color: '#555', fontSize: '10.5pt', marginTop: '4px' }}>
                    Budget: {fmt(Number(expense.researchGrant.awardedAmount))} &nbsp;|&nbsp; Spent: {fmt(Number(expense.researchGrant.spentAmount))}
                  </div>
                )}
              </div>
              ...
              <div style={{ marginBottom: '10px' }}>
                ...
                <span style={{ marginLeft: '12px' }}>{expense.researchProject ? expense.researchProject.projectCode : 'Not linked'}</span>
                {expense.researchProject && (
                  <div style={{ fontStyle: 'italic', color: '#555', fontSize: '10.5pt', marginTop: '4px' }}>{expense.researchProject.title}</div>
                )}
              </div>
              <div style={{ borderBottom: `1px solid ${BORDER}`, marginBottom: '10px' }} />
              <div>
                <span style={{ fontWeight: 'bold' }}>Submitted By:</span>
                <div style={{ marginTop: '2px' }}>{submitterName}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ border: `1px solid ${BORDER}`, borderRadius: '5px', overflow: 'hidden', marginBottom: '24px' }}>
          <div style={{ background: '#fafafa', color: NAVY, borderBottom: `1px solid ${BORDER}`, padding: '8px 12px', fontSize: '12pt', fontWeight: 'bold', display: 'flex', alignItems: 'center' }}>
            <IconEdit /> REMARK / ADDITIONAL NOTES
          </div>
        ...
        <div style={{ display: 'flex', gap: '80px', paddingLeft: '20px', paddingRight: '20px', marginBottom: '20px' }}>
          <div style={{ flex: 1, textAlign: 'center', marginTop: '40px' }}>
            ...
            <div style={{ fontSize: '10pt', fontStyle: 'italic', color: '#555', marginTop: '4px' }}>{submitterName}</div>
          </div>
          <div style={{ flex: 1, textAlign: 'center', marginTop: '40px' }}>
            <div style={{ borderBottom: `2px solid ${NAVY}`, marginBottom: '8px' }} />
            <div style={{ fontSize: '12pt', fontWeight: 'bold' }}>Approved by (Name & Sign)</div>
            <div style={{ fontSize: '10pt', fontStyle: 'italic', color: '#555', marginTop: '4px', minHeight: '18px' }}>
              {expense.approvedBy ? `${expense.approvedBy.firstName} ${expense.approvedBy.lastName}` : (expense.status === 'DRAFT' || expense.status === 'SUBMITTED' ? '(Draft)' : '')}
            </div>
          </div>
        </div>
...
PrintableExpenseReport.displayName = 'PrintableExpenseReport';
