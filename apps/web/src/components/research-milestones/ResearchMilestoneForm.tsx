...
const milestoneSchema = z.object({
  ...
  deliverableLink: z.string().url('Must be a valid URL').optional().or(z.literal('')),
});
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Planned Start <span className="text-slate-400 font-normal">(Optional)</span></label>
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Planned Due <span className="text-slate-400 font-normal">(Optional)</span></label>
      ...
      <div>
        <label className="block text-sm font-medium text-slate-700 mb-1">Responsible Member <span className="text-slate-400 font-normal">(Optional)</span></label>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Actual Completion Date <span className="text-slate-400 font-normal">(Optional)</span></label>
          <input type="date" {...register('actualCompletionDate')} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Deliverable Link <span className="text-slate-400 font-normal">(Optional)</span></label>
          <input type="text" placeholder="https://..." {...register('deliverableLink')} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          {errors.deliverableLink && <p className="text-red-500 text-xs mt-1">{errors.deliverableLink.message}</p>}
        </div>
      </div>
